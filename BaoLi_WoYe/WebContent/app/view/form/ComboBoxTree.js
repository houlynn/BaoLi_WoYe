...
Ext
		.define(
				"app.view.form.ComboBoxTree",
				{
					...
					// dropdown list's height,if tree's height was speciallized
					// ignore this
					...
					initComponent : function() {
						...
						/**
						 * @event treenodeselect Fires when the tree node is
						 *        selected
						 * @param {Ext.ux.form.ComboBoxTree}
						...
						/**
						 * @event beforecollapse Fires when the dropdown list
						 *        before collapsed
						 * @param {Ext.ux.form.ComboBoxTree}
						 */
						'beforecollapse',
						...
						/**
						 * @event beforeexpand Fires when the dropdown list
						 *        before expanded
						 * @param {Ext.ux.form.ComboBoxTree}
						 */
						'beforeexpand',
						...
						/**
						 * @event clearvalue Fires when the clear value trigger
						 *        is clicked
						 * @param {Ext.ux.form.ComboBoxTree}
						 * @param {String}
						...
						// user both do not specialized treePanel'height and
						// comboboxtree's listHeight,it's means dropdown list
						// auto fit height
						this.tree.height = Ext.isDefined(this.tree.height) ? this.tree.height
								: this.listHeight;
						...
						// if this.tree.height == undefined ,force to expand
						// tree'root if user do not specialized expanded = true
						this.tree.root.expanded = !Ext
								.isDefined(this.tree.height) ? true
								: this.tree.root.expanded;
					...
					onRender : function(ct, position) {
						...
						Ext.ux.form.ComboBoxTree.superclass.onRender.call(this,
								ct, position);

						if (this.hiddenName) {
							var formItem = this.el.findParent('.x-form-item',
									4, true);
							if (formItem) {
								// create a Ext.form.Hidden field as a sibling
								// of this, in order to
								// Ext.form.BasicForm.load(..)
					...
					initList : function() {
						if (!this.list) {
							var cls = 'x-combo-list', listParent = Ext
									.getDom(this.getListParent()
											|| Ext.getBody()), zindex = parseInt(
							...
							if (this.resizable) {
								...
								this.mon(this.resizer, 'resize', function(r, w,
										h) {
					...
					initEvents : function() {
						...
						Ext.ux.form.ComboBoxTree.superclass.initEvents
								.call(this);
						...
						this.tree.root.on({
							...
							'expand' : {
								fn : function() {
									if (!Ext.isDefined(this.tree.height)) {// dropdown
																			// list
																			// auto
																			// height
										this.restrictHeight();
					...
					// private
					initValue : function() {
						Ext.ux.form.ComboBoxTree.superclass.initValue
								.call(this);
						if (this.hiddenField) {
							this
									.setHiddenValue(Ext
					...
					getParentZIndex : function() {
						...
						if (this.ownerCt) {
							this.findParentBy(function(ct) {
								zindex = parseInt(ct.getPositionEl().getStyle(
										'z-index'), 10);
								return !!zindex;
							});
						}
					...
					restrictHeight : function() {
						...
						var inner = this.innerList.dom, pad = this.list
								...
								inner.scrollHeight), ha = this.getPosition()[1]
								- Ext.getBody().getScroll().top, hb = Ext.lib.Dom
								.getViewHeight()
								- ha - this.getSize().height, space = Math.max(
								ha, hb, this.minHeight || 0)
								- this.list.shadowOffset - pad;
					...
					// private
					onDestroy : function() {
						Ext.destroy(this.resizer, this.view, this.list);
						Ext.destroyMembers(this, 'hiddenField');
						Ext.ux.form.ComboBoxTree.superclass.onDestroy
								.call(this);
					},
					...
					setValue : function(val) {
						...
						Ext.ux.form.ComboBoxTree.superclass.setValue.call(this,
								val);
					},
					...
					collapse : function() {
						...
						if (this.fireEvent('beforecollapse', this) !== false) {
							this.list.hide();
							Ext.getDoc()
									.un('mousewheel', this.collapseIf, this);
					...
					expand : function() {
						...
						if (this.fireEvent('beforeexpand', this) !== false) {
							...
							// zindex can change, re-check it and set it if
							// necessary
							var listParent = Ext.getDom(this.getListParent()
									|| Ext.getBody()), zindex = parseInt(Ext
									.fly(listParent).getStyle('z-index'), 10);
							if (!zindex) {
								zindex = this.getParentZIndex();
							}
							if (zindex) {
					...
					collapseIf : function(e) {
						if (this.isExpanded() && !this.isDestroyed
								&& !e.within(this.wrap) && !e.within(this.list)) {
							this.isTreeNodeClicked = true;
							this.collapse();
						}
					},
					...
					// override, to prevent trigger hidden when set readOnly =
					// true
						...
						if (this.rendered) {
							if (this.readOnly) {
								...
								if (!this.editable) {
									this.el.dom.readOnly = true;
									this.el.addClass('x-trigger-noedit');
									this.mon(this.el, 'click',
											this.onTriggerClick, this);
								} else {
									this.el.dom.readOnly = false;
									this.el.removeClass('x-trigger-noedit');
									this.mun(this.el, 'click',
											this.onTriggerClick, this);
								}
